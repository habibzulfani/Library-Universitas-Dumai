	...
	testPapers := []models.Paper{
		{
			...
			Author:        "John Doe",
			Authors:       []models.PaperAuthor{{AuthorName: "John Doe"}},
			Advisor:       utils.StringPtr("Dr. Smith"),
			University:    utils.StringPtr("Sample University"),
			Department:    utils.StringPtr("Computer Science"),
			Year:          utils.IntPtr(2021),
			ISSN:          utils.StringPtr("1234-5678"),
			Language:      utils.StringPtr("English"),
			Journal:       utils.StringPtr("Sample Journal"),
			Volume:        utils.IntPtr(1),
			Issue:         utils.IntPtr(1),
			Pages:         utils.StringPtr("1-10"),
			DOI:           utils.StringPtr("10.1234/sample1"),
			Abstract:      utils.StringPtr("Abstract 1"),
			Keywords:      utils.StringPtr("keyword1, keyword2"),
			FileURL:       utils.StringPtr("/files/sample1.pdf"),
			CoverImageURL: utils.StringPtr("/covers/sample1.jpg"),
			CreatedBy:     utils.UintPtr(1),
		},
		{
			Title:         "Sample Paper 2",
			Author:        "Jane Smith",
			Authors:       []models.PaperAuthor{{AuthorName: "Jane Smith"}},
			Advisor:       utils.StringPtr("Dr. Brown"),
			University:    utils.StringPtr("Another University"),
			Department:    utils.StringPtr("Mathematics"),
			Year:          utils.IntPtr(2022),
			ISSN:          utils.StringPtr("8765-4321"),
			Language:      utils.StringPtr("English"),
			Journal:       utils.StringPtr("Another Journal"),
			Volume:        utils.IntPtr(2),
			Issue:         utils.IntPtr(2),
			Pages:         utils.StringPtr("11-20"),
			DOI:           utils.StringPtr("10.5678/sample2"),
			Abstract:      utils.StringPtr("Abstract 2"),
			Keywords:      utils.StringPtr("keyword3, keyword4"),
			FileURL:       utils.StringPtr("/files/sample2.pdf"),
			CoverImageURL: utils.StringPtr("/covers/sample2.jpg"),
			CreatedBy:     utils.UintPtr(2),
		},
	}
